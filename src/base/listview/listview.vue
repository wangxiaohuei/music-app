<template>
  <scroll class="listview" :data="data" :propType="propType" :listenScroll="listenScroll" ref="listview" @scroll="scroll">
    <ul>
...
export default {
  ...
  created() {
    // 写入 data 中，vue 会自动监听，因为我们不需要监听 touch 的变化，所以在这里定义
    this.touch = {}
    this.listenScroll = true
    this.listHeight = []
    this.propType = 3 // better-scroll 的属性表示实时更新
  },
  watch: {
    data() {
      setTimeout(() => {
  ...
  methods: {
    onShortcutStart(e) {
      ...
      this.touch.y1 = firstTouch.paggeY
      this.touch.anchorIndex = anchorIndex
      this._scrollTo(anchorIndex)
    },
    onShortcutTouchMove(e) {
      let firstTouch = e.touches[0]
      this.touch.y2 = firstTouch.paggeY
